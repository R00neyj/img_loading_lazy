# Project Overview
**img_loading_lazy** (v2.0.0) is a high-performance automated tool (CLI script) designed to improve web image loading performance and prevent Cumulative Layout Shift (CLS). It works by analyzing actual image files referenced in HTML, automatically adding explicit `width` and `height` attributes, and injecting optimization attributes like `loading="lazy"` and `decoding="async"`. 
